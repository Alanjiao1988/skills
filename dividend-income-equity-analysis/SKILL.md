...
- `visual-output-rules.md`：年度分红轨迹、逐年股息率、现金流覆盖桥、TTM vs normalized yield、三年分红跑道。完整分析必须读取。
- `withholding-notes.md`：预扣税和券商实测扣税规则，是税务处理唯一权威源。
- `scoring.md`：100 分评分模型和评分锚点。
- `output-template.md`：最终输出唯一章节结构来源。完整分析必须严格按该文件的 13 节输出；缺少 Dividend Snapshot、Dividend Trajectory、Cash-Flow Coverage Bridge、Three-Year Dividend Runway、Visual Summary 或 Required Ratings 视为输出不完整。
