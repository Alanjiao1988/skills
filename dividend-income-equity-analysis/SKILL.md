...
- `visual-output-rules.md`：能力分层渲染、三张标准图、年度分红轨迹、逐年股息率、现金流覆盖桥、TTM vs normalized yield、三年分红跑道。完整分析必须读取。
...
- `output-template.md`：最终输出唯一章节结构来源。完整分析必须严格按该文件的全部章节输出；缺少 Key Metrics at a Glance、Dividend Snapshot、Standard Charts 或 Text Fallback、Dividend Trajectory、Cash-Flow Coverage Bridge、Three-Year Dividend Runway、Expected Buy Zone、Visual Summary 或 Required Ratings 视为输出不完整。
...
- 必须根据输出能力提供三张标准图或文本降级视图。
- 必须用三年情景表展示未来 DPS、净股息率、预计现金流、分红现金支出和覆盖倍数。
- 必须基于 normalized net DPS、目标税后收益率、历史股息率区间和安全边际推导预期买入区间；不得把高 TTM 股息率直接当作买入依据。
